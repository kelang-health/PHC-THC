...
function renderPanel(){
  ...
  p.querySelector('[data-perf-copy]')?.addEventListener('click',async e=>{const ok=await copyReport();e.currentTarget.textContent=ok?'คัดลอกแล้ว':'คัดลอกไม่ได้';});p.querySelector('[data-perf-clear]')?.addEventListener('click',()=>{state.nav=[];state.resources=[];state.requests=[];state.longTasks=[];state.performanceKeys.clear();state.activeNav=null;if(Array.isArray(window.PHCHealthCoordTrace))window.PHCHealthCoordTrace.length=0;performance.clearResourceTimings?.();renderPanel();});
}
if(ENABLED){window.PHCPerfDiagnostics={report,clear(){state.nav=[];state.resources=[];state.requests=[];state.longTasks=[];state.performanceKeys.clear();state.activeNav=null;if(Array.isArray(window.PHCHealthCoordTrace))window.PHCHealthCoordTrace.length=0;performance.clearResourceTimings?.();renderPanel();},disable(){sessionStorage.removeItem('phc.perf.enabled');location.reload();}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{captureBufferedResources();ensurePanel();renderPanel();},{once:true});else{captureBufferedResources();ensurePanel();renderPanel();}window.addEventListener('load',()=>setTimeout(()=>{captureBufferedResources();renderPanel();},150),{once:true});setTimeout(()=>{captureBufferedResources();renderPanel();},1200);}
