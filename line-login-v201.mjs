...
import { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY } from './config.js?v=2.0.2';
...
async function startLineLogin(){
  ...
  try{
    ...
    state.innerHTML=`<p class="line-login-status" id="line-login-status">เปิด LINE OA ของหน่วยงานที่เคยเชื่อมกับ OSM-PHC แล้วส่งข้อความนี้ภายใน <span class="line-login-count" id="line-login-countdown">3:00</span></p><div class="line-login-code" id="line-login-code">LOGIN ${String(data.code||'')}</div><div class="line-login-actions"><button type="button" class="secondary" id="line-login-copy">คัดลอกรหัส</button><button type="button" class="secondary" id="line-login-cancel">ยกเลิก</button></div><p class="line-login-status">เมื่อ LINE ตอบว่าอนุมัติแล้ว หน้านี้จะเข้าสู่ระบบอัตโนมัติ ไม่ต้องกรอกรหัสผ่าน</p>`;
