...
function openMemberRequest(houseId,onDone){
  ...
  body.innerHTML=`<form class="phc190-form" data-member-form><label>เลขประจำตัวประชาชน 13 หลัก<input name="cid" inputmode="numeric" pattern="[0-9]*" maxlength="13" autocomplete="off" required></label><label>ชื่อ<input name="first_name" maxlength="80" autocomplete="off" required></label><label>นามสกุล<input name="last_name" maxlength="100" autocomplete="off" required></label><label>วันเดือนปีเกิด<input name="birth" type="date" max="${new Date().toISOString().slice(0,10)}" required></label><div class="phc190-note">หากพบ CID เดิม ระบบจะส่งให้ Admin ตรวจเชื่อมกับ PID เดิมแทนการสร้างคนซ้ำ · เมื่อเชื่อมสำเร็จ หน้านี้จะแสดง PID แทนเลขบัตร</div><div class="phc190-error" data-err></div><button class="phc190-primary">ตรวจสอบและส่งคำขอ</button></form>`;
