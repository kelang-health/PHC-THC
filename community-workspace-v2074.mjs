...
export function houseCards(rows, {assignmentBadge, emptyText = 'ไม่พบรายการบ้านตามสิทธิ์', indexOffset = 0} = {}) {
  ...
  return `<div class="community-record-list">${rows.map((h, index) => {
    ...
    return `<article class="community-record-card" data-community-house="${esc(h.id)}">
      <header><span class="community-record-index">${indexOffset + index + 1}</span><div><h5>บ้าน ${esc(h.house_no || 'ไม่ระบุ')}</h5><p>หมู่ ${esc(h.moo || '—')} · ${esc(h.community || '—')} · HCODE ${esc(h.hcode || '—')}</p></div></header>
